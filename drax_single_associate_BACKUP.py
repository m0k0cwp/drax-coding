...
# CONFIGURATION
# ===========================================================================
TARGET_ASSOCIATE_NAME = "GUERBY ELIASSAINT"   # Name to search for on the page
DATE             = "2026-04-01"
SHIFT            = "S1"
...
def find_associate_href(page, name: str) -> str | None:
    ...
    first_name   = name.strip().split()[0].lower()   # e.g. 'krista'
    ...
    # ---- Dump all inputs so we know what's on the page ----
    ...
    # ---- Try DataTables JS API first (fastest) ----
    try:
        ...
        """, name)
        if found_via_js:
            print(f"    Search triggered via JS ({found_via_js}) for '{name}'")
    ...
    try:
        ...
        if inp.is_visible(timeout=3_000):
            ...
            inp.type(name, delay=80)
            page.wait_for_timeout(2_000)
            print(f"    Playwright typed into #table-search for '{name}'")
    except Exception:
        pass
